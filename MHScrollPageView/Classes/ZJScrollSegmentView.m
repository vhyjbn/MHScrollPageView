...
- (void)addScrollLineOrCoverOrExtraBtn {
    if (self.segmentStyle.isShowLine) {
        UIView *separateLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.zj_height - 0.5, self.frame.size.width, 0.5)];
        
        separateLine.backgroundColor = (nil != self.segmentStyle.seperateLineColor) ? self.segmentStyle.seperateLineColor : [[UIColor lightGrayColor] colorWithAlphaComponent:0.5];
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        self.scrollLine.layer.cornerRadius = self.segmentStyle.scrollLineHeight / 2.0;
        self.scrollLine.layer.masksToBounds = true;
